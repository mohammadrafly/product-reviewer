        ...
        @foreach ($data['product'] as $item)
            @if ($counter < 5)
                <div class="hidden duration-700 ease-in-out" data-carousel-item>
                    <img src="{{ asset('product_pictures/'. $item['product_picture'])}}" class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="...">
                </div>
            @else
                @break
            @endif
...
<div class="grid grid-cols-2 md:grid-cols-3 gap-4">
    @foreach ($data['product'] as $product)
        <a href="{{ route('single.product', $product->id)}}">
            ...
            <div class="text-xl mt-2 font-thin">{{ $product->product_name }}</div>
            <div class="flex items-center mt-5">
                @php
                    $productReview = $data['review']->where('id_product', $product->id)->avg('stars');
